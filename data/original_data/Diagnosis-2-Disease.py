...
class Diagnosis2Diseases(object):
    ...
    def process_text(self, data):
        ...
        """
        process_data = []
        icd_9_dis = ICD_9_Ontology()  # 声明ICD-9疾病本体对象
        ...
        for line in tqdm(data, desc='处理数据中...', leave=False):
            ...
            if 'E' not in icd9_code:
                if len(icd9_code) >= 4:
                    icd9_code = icd9_code[:3] + '.' + icd9_code[3:]
            else:
                if len(icd9_code) > 4:
                    icd9_code = icd9_code[:4] + '.' + icd9_code[4:]
            one.append(icd9_code)
            disease = icd_9_dis.get_concept_by_cid(icd9_code)  # 获取ICD-9码对应的文本
            one.append(self._normalize_text(disease.get()[1]))
            disease_fathers = disease.get_all_ancestor()  # 回溯ICD-9码的父概念及其文本
            for father in disease_fathers:
...
dia_2_dis = Diagnosis2Diseases(1)  # 数据类对象
dia_2_dis_data = dia_2_dis.query_postgres()  # 查询数据库
process_data = dia_2_dis.process_text(dia_2_dis_data)  # 处理文本
file_path = os.getcwd() + '\\'
...
output = open((file_path + 'diagnosis-2-disease' + '.csv'), mode='w', newline='', encoding='utf-8')  # 写成.csv文件
